...
% Assign data depending on type of frequency dependence

if isstruct(data) % Name is given, else, data is supplied
    if strcmp('fi4',data.type) || strcmp('FI4',data.type)
        data = [4,4,1,0,0,0,1.0,inf];
        def = 1;
    elseif strcmp('fi9',data.type) || strcmp('FI9',data.type)
        data = [9,9,1,0,0,0,1.0,inf];
        def = 1;
    elseif strcmp('Tucker',data.type) || strcmp('tucker',data.type)
        data=[6.97,9.77,2*pi/data.Tp,4.06,-1.91,0,1.05,inf];
        def = 1;
    elseif strcmp('Mitsuyasu',data.type) || strcmp('mitsuyasu',data.type)
        data=[11.5,11.5,2*pi/data.Tp,5,-2.5,0,1.0,inf];
        def = 1;
    elseif strcmp('Ewans',data.type) || strcmp('ewans',data.type)
        data=[15.5,13.1,2*pi/data.Tp,9.47,-1.94,0,1.0,inf];
        def = 1;
    end
